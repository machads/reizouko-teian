...
function createRecipePrompt(ingredients, requiredSeasoning, moodRequest) {
    ...
    const seasoningText = requiredSeasoning ? `調味料: ${requiredSeasoning}` : '';
    const moodText = moodRequest ? `要望: ${moodRequest}` : '';
    
    return `食材: ${ingredientsList}
${seasoningText}
${moodText}
    ...
    try {
        ...
        try {
            ...
            messages: [
                {
                    role: "system",
                    content: `プロの料理人として、指定食材で和風・洋風・中華の3つのレシピを提案してください。各レシピは簡潔で実用的に。必ず【和風料理】【洋風料理】【中華料理】の形式で回答。`
                },
                {
                    role: "user",
                    content: prompt
